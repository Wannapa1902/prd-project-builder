...
import { DEPARTMENTS, STATUSES, formatDate, statusLabel, workCode } from "@/lib/constants";
...
function MeetingSummaryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (department !== "all") list = list.filter((w) => w.department === department);
  ...
  const markdown = useMemo(() => {
    ...
    lines.push(
      `วันที่ประชุม: ${formatDate(to)} (ข้อมูลช่วง ${formatDate(from)} - ${formatDate(to)})`,
    );
    lines.push("");
    ...
    completed.forEach((w) =>
      lines.push(
        `- [${workCode(w.work_no)}] ${w.title} - แผนก: ${w.department ?? "-"} | ผู้รับผิดชอบ: ${w.owner_name ?? "-"}`,
      ),
    );
    lines.push("");
    ...
    inProgress.forEach((w) => {
      ...
      lines.push(
        `  แผนก: ${w.department ?? "-"} | สถานะ: ${statusLabel(w.status)} | ความคืบหน้า: ${w.progress}%`,
      );
    ...
    issues.forEach((w) => {
      ...
      lines.push(`  แผนก: ${w.department ?? "-"} | ผู้รับผิดชอบ: ${w.owner_name ?? "-"}`);
    });
    lines.push("");
    lines.push("## 4. Action Items");
    lines.push("| Action | แผนก | ผู้รับผิดชอบ | กำหนดเสร็จ |");
    lines.push("|---|---|---|---|");
    actionItems.forEach((w) =>
      lines.push(
        `| ${w.next_action} | ${w.department ?? "-"} | ${w.owner_name ?? "-"} | ${formatDate(w.due_date)} |`,
      ),
    );
    return lines.join("\n");
    ...
    <AppLayout>
      <div className="mx-auto max-w-4xl">
        ...
        <p className="mb-6 text-sm text-muted-foreground">
          เลือกช่วงเวลาและเงื่อนไข แล้วคัดลอกรายงานไปใช้ประชุมได้ทันที
        </p>
        ...
        <div className="mb-3 flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            พบ {filtered.length} งานที่อัพเดทในช่วงเวลานี้
          </p>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">ตัวอย่างรายงาน</CardTitle>
          </CardHeader>
          <CardContent>
